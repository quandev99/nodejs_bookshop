...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const btnDeleteUser = document.getElementById("btn-delete-comment");
  ...
  $("#delete-comment-modal").on("show.bs.modal", function (event) {
    ...
    btnDeleteUser.onclick = async () => {
      ...
      const response = await fetch(
        `http://localhost:1999/comment/${userId}/deleteComment`,
        {
          method: "DELETE",
          headers: {
      ...
      if (data) {
        location.href = "/admin/me/stored/listComments";
        return;
      } else {
        alert(data.message);
      }
